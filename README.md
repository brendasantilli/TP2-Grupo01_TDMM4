[sketch.js](https://github.com/user-attachments/files/31891484/sketch.js)
let sistema;
let globalX = 0; let globalY = 0;
let videos = [];
let nombres = [
  'memoria', 'herencia', 'caducidad', 
  'identidad', 'empatia', 'colaboracion', 
  'incertidumbre', 'ansiedad', 'expectativa'
];

function preload() {
  for (let i = 0; i < 9; i++) {
    // Cargamos los videos sin depender del callback asíncrono en preload
    videos[i] = createVideo([`assets/${nombres[i]}.mp4`]);
    videos[i].hide();
  }
}
...
function setup() {
  ...
  // Configuración directa y segura de cada video nativo en el DOM
  for (let i = 0; i < videos.length; i++) {
    ...
    if (vid && vid.elt) {
      ...
      // Reinicio forzado por evento nativo al finalizar
...
function touchStarted() {
  // Desbloquear/reproducir los videos ante cualquier toque o clic del usuario
  for (let v of videos) {
    if (v && v.elt) {
      v.elt.muted = true;
      v.elt.loop = true;
  ...
  dibujarGrilla() {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      if (vid && vid.elt) {
        ...
        // Comprobación cuadro a cuadro: si el video terminó o se pausó, reiniciarlo
        if (elem.ended || (elem.duration > 0 && elem.currentTime >= elem.duration - 0.1)) {
          elem.currentTime = 0;
          elem.play().catch(() => {});
        } else if (elem.paused) {
          elem.play().catch(() => {});
        ...
        if (elem.readyState >= 2) {
          push();
          imageMode(CORNER); // Alineado desde la esquina superior izquierda
          image(vid, x, y, tx, ty); // Ocupa todo el ancho (tx) y alto (ty) del cuadrante
          pop();
        }
      }
    }

    // Dibujar las líneas de la grilla por encima para un acabado limpio
    line(tx, 0, tx, height); 
...
function aplicarEstilo(colorHex, estilo, opacidad = 255) {
  ...
  else if (estilo === 1) { noFill(); stroke(c); strokeWeight(8); drawingContext.setLineDash([]); }
...
class ConceptoMemoria {
  constructor() {
    ...
    this.c1 = new Cuadrado(width * 0.20, height * 0.30, tB * 0.75, COLORES[0], 1);
    this.c2 = new Cuadrado(width * 0.75, height * 0.40, tB * 1.20, COLORES[1], 0);
    this.c3 = new Cuadrado(width * 0.40, height * 0.75, tB * 0.95, COLORES[2], 2);
    this.rastros = []; 
  ...
  update() { 
    for (let i = this.rastros.length - 1; i >= 0; i--) {
      ...
      if (!r.activo) {
        r.opacidad -= 0.85; 
        if (r.opacidad <= 0) this.rastros.splice(i, 1);
      }
    }
  }
  ...
  display() {
    for (let r of this.rastros) {
      if (r.puntos.length > 1) { 
        noFill(); 
        if (r.id === 0) { stroke(26, 26, 26, r.opacidad); strokeWeight(8); drawingContext.setLineDash([]); }
        else if (r.id === 1) { stroke(22, 78, 150, r.opacidad); strokeWeight(25); drawingContext.setLineDash([]); }
        else if (r.id === 2) { stroke(212, 42, 42, r.opacidad); strokeWeight(3); drawingContext.setLineDash([15, 15]); }
    ...
    this.c2.display(); this.c3.display(); this.c1.display();
  }
  ...
  soltar() { 
    ...
    if (this.rastroActual) { this.rastroActual.activo = false; this.rastroActual = null; this.figuraActiva = null; }
  }
}
...
class ConceptoCaducidad {
  ...
  presionar(tx, ty) {
    ...
    for (let f of this.figuras) { if (f.tocando(tx, ty)) f.arrastrando = true; }
  }
  ...
  soltar() { for (let f of this.figuras) f.arrastrando = false; }
}
...
class ConceptoIdentidad {
  ...
  arrastrar(tx, ty) { if (this.presionado) { this.tiempoEfecto = 45; } }
  soltar() { this.presionado = false; }
}
...
class ConceptoColaboracion {
  constructor() {
    let tB = min(width, height) * 0.35;
    this.circulos = [
      new Circulo(width * 0.25, height * 0.3, tB * 0.75, COLORES[0], 1),
      new Circulo(width * 0.75, height * 0.35, tB * 1.20, COLORES[1], 0),
      new Circulo(width * 0.5, height * 0.75, tB * 0.95, COLORES[2], 2)
    ];
  }
  update() {}
  display() { for (let c of this.circulos) c.display(); }
  presionar(tx, ty) { for (let c of this.circulos) if (c.tocando(tx, ty)) c.arrastrando = true; }
  arrastrar(tx, ty) { for (let c of this.circulos) if (c.arrastrando) { c.x = tx; c.y = ty; } }
  soltar() { for (let c of this.circulos) c.arrastrando = false; }
}
...
class ConceptoIncertidumbre {
  constructor() {
    let tB = min(width, height) * 0.35;
    this.figuras = [
      new Triangulo(width * 0.3, height * 0.3, tB * 0.75, COLORES[0], 1),
      new Cuadrado(width * 0.7, height * 0.4, tB * 1.20, COLORES[1], 0),
      new Circulo(width * 0.5, height * 0.75, tB * 0.95, COLORES[2], 2)
    ];
  }
  update() {}
  display() { for (let f of this.figuras) f.display(); }
  presionar(tx, ty) { for (let f of this.figuras) if (f.tocando(tx, ty)) f.arrastrando = true; }
  arrastrar(tx, ty) { for (let f of this.figuras) if (f.arrastrando) { f.x = tx; f.y = ty; } }
  soltar() { for (let f of this.figuras) f.arrastrando = false; }
}
...
class ConceptoAnsiedad {
  constructor() {
    let tB = min(width, height) * 0.35;
    this.figuras = [
      new Cuadrado(width * 0.25, height * 0.3, tB * 0.75, COLORES[0], 1),
      new Triangulo(width * 0.75, height * 0.35, tB * 1.20, COLORES[1], 0),
      new Circulo(width * 0.5, height * 0.7, tB * 0.95, COLORES[2], 2)
    ];
  }
  update() {}
  display() { for (let f of this.figuras) f.display(); }
...
class ConceptoExpectativa {
  constructor() {
    let tB = min(width, height) * 0.35;
    this.figuras = [
      new Circulo(width * 0.3, height * 0.3, tB * 0.75, COLORES[0], 1),
      new Cuadrado(width * 0.7, height * 0.35, tB * 1.20, COLORES[1], 0),
      new Triangulo(width * 0.5, height * 0.7, tB * 0.95, COLORES[2], 2)
    ];
  }
  update() {}
  display() { for (let f of this.figuras) f.display(); }
